...
it('Should todos be a function', () => {
  expect(todos).be.a('function')
})

it('Should add a todo item', () => {
  const before = deepFreeze(initialState)
  const action = deepFreeze({ type: ADD_TODO, payload: { id: 0, text: 'Hey' } })
...
it('Should add a new todo item', () => {
  ...
  const action = deepFreeze({ type: ADD_TODO, payload: { id: 1, text: 'Ho' } })
  const after = [
...
it('Should toggle first todo', () => {
  ...
  const action = deepFreeze({ type: TOGGLE_TODO, payload: { id: 0 } })
  const after = [
...
it('Should toggle second todo', () => {
  ...
  const action = deepFreeze({ type: TOGGLE_TODO, payload: { id: 1 } })
  const after = [
    { id: 0, text: 'Hey', completed: true },
    { id: 1, text: 'Ho', completed: false }
  ]
